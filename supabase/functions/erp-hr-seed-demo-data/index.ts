 ...
 * Justified: It is a seed/demo-only function that populates initial data.
...
serve(async (req) => {
  ...
  try {
    // === AUTH GATE: JWT + Admin Role Check ===
    ...
    if (claimsError || !claimsData?.claims?.sub) {
      console.error('[erp-hr-seed-demo-data] Auth error:', claimsError?.message);
      return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (!roleData) {
      console.warn(`[erp-hr-seed-demo-data] Forbidden: user ${userId} lacks admin role`);
