...
| 3c | Stock options | Equity · Payroll | sí | sí | sí | parcial | sí (engine + E2E) | PARTIAL | PARTIAL · HUMAN_REVIEW_REQUIRED (C2: payslipEngine + ssContributionEngine + payslipRenderModel + modelo190PipelineEngine reales invocados E2E; ES_STOCK_OPTIONS aparece como devengo real, cotiza, impacta baseIRPF y agregación 190; pendientes: clave/subclave 190 dedicada, motor real de cost report, computeIRPF real) | no | sí | línea SO + acumulado 190 | C2 missingCapabilities: modelo190_so_clave_dedicada, cost_report_engine, irpf_real_compute |
...
| 8g | Informe fiscal 111/190 | Fiscal · Reporting | sí | sí | sí | DRYRUN AEAT | parcial | PARTIAL | PARTIAL · HUMAN_REVIEW_REQUIRED (C2: aggregatePerceptorsForModelo190 real cuadra percepciones_integras y retenciones_practicadas con payslip real; clave/subclave SO dedicada no expuesta por engine actual) | sí (AEAT) | sí (clasificación SO) | acumulados + claves | — |
...
| 11 | Modelos 111/190 con SO y flex | Fiscal | parcial | sí | sí | DRYRUN | parcial | PARTIAL | PARTIAL · HUMAN_REVIEW_REQUIRED (C2: pipeline 190 real agrega bases/retenciones de payslip real con SO; clave dedicada SO sigue ausente — gap mapping ES_STOCK_OPTIONS → clave 190) | sí (AEAT) | sí (SO) | acumulados + claves/subclaves | — |
...
| 14 | Recibo final imprimible | Payroll · Render | sí | sí | sí | render model + PDF | parcial | PARTIAL | PARTIAL (C2: buildPayslipRenderModel real expone línea ES_STOCK_OPTIONS desde catálogo real; PDF binario sigue sin generarse en tests) | no | no | totales coherentes + conceptos | C1 valida estructura, C2 confirma render model real con catálogo real |
