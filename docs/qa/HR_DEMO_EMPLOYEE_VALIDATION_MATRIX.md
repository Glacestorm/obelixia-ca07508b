...
| 8g | Informe fiscal 111/190 | Fiscal · Reporting | sí | sí | sí | DRYRUN AEAT | parcial | PARTIAL | PASS_INTERNAL · HUMAN_REVIEW_REQUIRED (C3: pipeline 190 ahora rechaza A/01 silencioso para SO — emite `clave_is_fallback`, `requires_human_review`, `official_submission_blocked` y warning en `qualityReport.issues`; clave/subclave fiscal SO permanece como decisión humana documentada en catálogo) | sí (AEAT) | sí (clasificación SO) | acumulados + claves + review flag | C3: stock-options-modelo190-key.test.ts |
...
| 11 | Modelos 111/190 con SO y flex | Fiscal | parcial | sí | sí | DRYRUN | parcial | PARTIAL | PASS_INTERNAL · HUMAN_REVIEW_REQUIRED (C3: SO en breakdown bloquea presentación oficial vía `official_submission_blocked=true`; CCAA foral País Vasco/Navarra produce `limitations` honestas en `computeIRPF` real → HUMAN_REVIEW_REQUIRED, no PASS) | sí (AEAT) | sí (SO + foral) | acumulados + claves + review flag | C3: stock-options-real-irpf.test.ts |
