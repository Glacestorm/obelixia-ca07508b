...
export function useEnergyAlerts(companyId: string) {
  ...
  const computeAlerts = useCallback(async () => {
    ...
    try {
      ...
      // Parallel: checklists, workflow, proposals
      ...
      for (const c of cases) {
        ...
        // 3. Workflow stalled (>15 days same status)
        ...
        // 4. Stale proposals (sent > 10 days without response)
        const caseProposals = (proposalRes.data || []).filter(p => p.case_id === c.id);
        for (const p of caseProposals) {
          if (['issued', 'sent'].includes(p.status) && p.issued_at) {
          ...
          // Check expired
          if (['issued', 'sent'].includes(p.status) && p.valid_until) {
  ...
  return { alerts, loading, refresh: computeAlerts };
}
