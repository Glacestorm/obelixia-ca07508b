...
import {
  Plus, FileText, CheckCircle2, XCircle, Send, Eye, Clock
} from 'lucide-react';
...
export function CaseProposalTab({ caseId }: Props) {
  const { proposals, loading, createProposal, updateProposal, acceptProposal, rejectProposal, issueProposal } = useEnergyProposals(caseId);
  ...
  const handleCreate = useCallback(async () => {
    const savings = parseFloat(form.estimated_annual_savings) || 0;
    await createProposal({
      ...
      estimated_annual_savings: savings || null,
  ...
  const handleReject = useCallback(async () => {
    if (showReject) {
      await rejectProposal(showReject, rejectReason);
      setShowReject(null);
      setRejectReason('');
    }
  }, [showReject, rejectReason, rejectProposal]);
            ...
            <Card key={p.id}>
              ...
              <CardContent className="space-y-3">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                  ...
                  {p.valid_until && <div><span className="text-muted-foreground text-xs">Válida hasta:</span><p>{fmtDate(p.valid_until)}</p></div>}
                </div>

                {p.conditions && (
                  <div><span className="text-xs text-muted-foreground">Condiciones:</span><p className="text-sm">{p.conditions}</p></div>
                )}
                {p.observations && (
                  <div><span className="text-xs text-muted-foreground">Observaciones:</span><p className="text-sm">{p.observations}</p></div>
                )}
                ...
                <PermissionGate action="approve_recommendation">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {p.status === 'draft' && (
                      <Button size="sm" variant="outline" onClick={() => issueProposal(p.id)}>
                    ...
                    {['issued', 'sent'].includes(p.status) && (
                      <>
                        ...
                        <Button size="sm" variant="destructive" onClick={() => setShowReject(p.id)}>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-4 py-2">
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <Input value={form.cups} onChange={e => setForm(f => ({ ...f, cups: e.target.value }))} placeholder="ES00..." />
              </div>
              <div className="grid gap-2">
                <Label className="text-xs">Comercializadora actual</Label>
                <Input value={form.current_supplier} onChange={e => setForm(f => ({ ...f, current_supplier: e.target.value }))} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label className="text-xs">Tarifa actual</Label>
                <Input value={form.current_tariff} onChange={e => setForm(f => ({ ...f, current_tariff: e.target.value }))} />
              </div>
              <div className="grid gap-2">
                <Label className="text-xs">Coste anual actual (€)</Label>
                <Input type="number" step="0.01" value={form.current_annual_cost} onChange={e => setForm(f => ({ ...f, current_annual_cost: e.target.value }))} />
              </div>
            </div>
            <Separator />
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label className="text-xs">Comercializadora recomendada</Label>
                <Input value={form.recommended_supplier} onChange={e => setForm(f => ({ ...f, recommended_supplier: e.target.value }))} />
              </div>
              <div className="grid gap-2">
                <Label className="text-xs">Tarifa recomendada</Label>
                <Input value={form.recommended_tariff} onChange={e => setForm(f => ({ ...f, recommended_tariff: e.target.value }))} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label className="text-xs">Coste estimado anual (€)</Label>
                <Input type="number" step="0.01" value={form.estimated_annual_cost} onChange={e => setForm(f => ({ ...f, estimated_annual_cost: e.target.value }))} />
              </div>
              <div className="grid gap-2">
                <Label className="text-xs">Ahorro estimado anual (€)</Label>
                <Input type="number" step="0.01" value={form.estimated_annual_savings} onChange={e => setForm(f => ({ ...f, estimated_annual_savings: e.target.value }))} />
              </div>
            </div>
            <div className="grid gap-2">
              <Label className="text-xs">Condiciones</Label>
              <Textarea value={form.conditions} onChange={e => setForm(f => ({ ...f, conditions: e.target.value }))} placeholder="Permanencia, penalización..." rows={2} />
            </div>
            <div className="grid gap-2">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreate} disabled={!form.recommended_supplier}>Crear propuesta</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!showReject} onOpenChange={() => setShowReject(null)}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleReject} disabled={!rejectReason.trim()}>Confirmar rechazo</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
