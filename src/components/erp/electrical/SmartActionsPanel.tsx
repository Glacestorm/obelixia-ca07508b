/**
 * SmartActionsPanel - Next-best-action suggestions with priority, urgency, risk
 */
...
import {
  ...
  Loader2, Lightbulb, Clock, TrendingUp, Shield
} from 'lucide-react';
...
const URGENCY_CONFIG: Record<string, { label: string; variant: 'default' | 'secondary' | 'destructive' | 'outline'; icon: typeof AlertTriangle }> = {
  critical: { label: 'Crítica', variant: 'destructive', icon: AlertTriangle },
  high: { label: 'Alta', variant: 'destructive', icon: Clock },
  medium: { label: 'Media', variant: 'secondary', icon: Lightbulb },
  low: { label: 'Baja', variant: 'outline', icon: TrendingUp },
};
                    ...
                    <div className="flex items-start gap-3">
                      <div className={cn(
                        ...
                        <ActionIcon className={cn(
                          "h-4 w-4",
                          action.urgency === 'critical' ? 'text-destructive' : 'text-primary'
                        )} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-sm font-medium truncate">{action.title}</span>
                          <Badge variant={urgencyInfo.variant} className="text-[9px] shrink-0">
                            {urgencyInfo.label}
                          </Badge>
                        </div>
                        {action.description && (
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          {onNavigateToCase && (
                            <Button
                              ...
                              onClick={() => onNavigateToCase(action.case_id)}
                            >
                          ...
                          >
                            {completing === action.id ? (
                              ...
                              <><CheckCircle2 className="h-3 w-3 mr-1" /> Hecho</>
                            )}
                          </Button>
                        </div>
                      </div>
