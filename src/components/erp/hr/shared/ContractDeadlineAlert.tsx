/**
 * ContractDeadlineAlert — Inline deadline signal for ContractDataPanel
 * V2-ES.6 Paso 1.2: Shows SEPE communication deadline status
 *
 * Pure component — receives computed deadlines by prop.
 * Mirrors RegistrationDeadlineAlert pattern.
 */
import { AlertTriangle, AlertOctagon, Clock, CheckCircle2, Ban } from 'lucide-react';
import { cn } from '@/lib/utils';
import type { ContractDeadlineSummary, ContractDeadline } from './contractDeadlineEngine';

interface Props {
  summary: ContractDeadlineSummary;
  className?: string;
}
...
function DeadlineRow({ deadline }: { deadline: ContractDeadline }) {
...
export function ContractDeadlineAlert({ summary, className }: Props) {
    ...
    return (
      <div className={cn('rounded-lg border px-3 py-2 space-y-1', worst.bg, className)}>
        <div className={cn('flex items-center gap-1.5 text-[10px] font-medium', worst.color)}>
          ...
          <span>Sin fecha de inicio — no se pueden calcular plazos SEPE</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={cn('rounded-lg border px-3 py-2 space-y-1', worst.bg, className)}>
      <div className={cn('flex items-center gap-1.5 text-[10px] font-medium', worst.color)}>
        ...
        <span>Plazos comunicación SEPE</span>
      </div>
