/**
 * TGSSPreIntegrationBadge — Compact pre-integration status for registration panel
 * V2-ES.5 Paso 3: Shows unified preparation status with consistency details
 *
...
import type { PreIntegrationSummary, PreIntegrationStatus } from './tgssPreIntegrationReadiness';
...
const STATUS_STYLES: Record<PreIntegrationStatus, { icon: typeof Shield; color: string; bg: string }> = {
  tgss_prepared:          { icon: CheckCircle2, color: 'text-emerald-700', bg: 'bg-emerald-500/10 text-emerald-700 border-emerald-500/30' },
...
export function TGSSPreIntegrationBadge({ summary, showDetails = false, className }: Props) {
  ...
  return (
    <div className={cn('space-y-1.5', className)}>
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] text-muted-foreground">Pre-integración:</span>
      ...
      {showDetails && (
        <div className="space-y-0.5 pl-5">
          ...
          {summary.consistency.errorCount === 0 && summary.consistency.warningCount === 0 &&
            summary.payload.consistency.infos.length > 0 && (
            summary.payload.consistency.infos.slice(0, 2).map((info, i) => (
