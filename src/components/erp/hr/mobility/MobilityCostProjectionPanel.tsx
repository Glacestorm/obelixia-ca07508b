        ...
        <div className="flex items-center justify-between">
          ...
          <Dialog open={showForm} onOpenChange={setShowForm}>
            <DialogTrigger asChild>
              <Button size="sm" variant="outline" className="text-xs h-7"><Plus className="h-3 w-3 mr-1" />Añadir año</Button>
            </DialogTrigger>
            <DialogContent className="max-w-lg">
              <DialogHeader><DialogTitle className="text-sm">Proyección de coste anual</DialogTitle></DialogHeader>
              <div className="space-y-3">
                <div>
                  <Label className="text-xs">Año</Label>
                  <Input type="number" value={form.projection_year} onChange={e => setForm(p => ({ ...p, projection_year: Number(e.target.value) }))} className="h-8 text-sm" />
                </div>
