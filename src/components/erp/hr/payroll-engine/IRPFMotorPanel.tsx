...
export function IRPFMotorPanel({ companyId }: IRPFMotorPanelProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card><CardContent className="p-4"><div className="flex items-center gap-2 mb-1"><Users className="h-4 w-4 text-primary" /><span className="text-xs text-muted-foreground">Perceptores</span></div><p className="text-2xl font-bold">3</p></CardContent></Card>
        <Card><CardContent className="p-4"><div className="flex items-center gap-2 mb-1"><Calculator className="h-4 w-4 text-primary" /><span className="text-xs text-muted-foreground">Tipo medio</span></div><p className="text-2xl font-bold">19.6%</p></CardContent></Card>
        <Card><CardContent className="p-4"><div className="flex items-center gap-2 mb-1"><TrendingUp className="h-4 w-4 text-primary" /><span className="text-xs text-muted-foreground">Retención trimestral</span></div><p className="text-2xl font-bold">5.356,50 €</p></CardContent></Card>
        <Card><CardContent className="p-4"><div className="flex items-center gap-2 mb-1"><AlertTriangle className="h-4 w-4 text-amber-500" /><span className="text-xs text-muted-foreground">Regularizaciones</span></div><p className="text-2xl font-bold">1</p></CardContent></Card>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <div key={emp.id} className="p-4 rounded-lg border bg-card hover:bg-muted/50 transition-colors">
                      <div className="flex items-center justify-between mb-2">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">Situación {emp.situacion} · Base anual: {emp.baseAnual.toLocaleString('es-ES')} €</p>
                        </div>
                        <div className="flex items-center gap-2">
                          {emp.regularizado && <Badge variant="outline" className="text-[10px] border-amber-500 text-amber-600">Regularizado</Badge>}
                          {emp.tipoSolicitado && emp.tipoSolicitado > emp.tipoLegal && <Badge variant="secondary" className="text-[10px]">Art. 88.5</Badge>}
                        </div>
                      </div>
        ...
        <TabsContent value="modelo111" className="mt-4">
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div><CardTitle className="text-base">Modelo 111 — Retenciones e ingresos a cuenta</CardTitle><CardDescription>Declaración trimestral de retenciones IRPF (rendimientos del trabajo)</CardDescription></div>
                <Button size="sm" onClick={() => toast.success('Modelo 111 generado')}><Download className="h-4 w-4 mr-1" /> Generar</Button>
              </div>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Nº perceptores</span><span className="font-mono">3</span></div>
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Base retenciones</span><span className="font-mono">26.250,00 €</span></div>
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Retenciones</span><span className="font-mono font-semibold text-primary">5.356,50 €</span></div>
                  </div>
                </div>
                <div className="space-y-4">
                  <h4 className="text-sm font-semibold">Resumen</h4>
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm"><span className="text-muted-foreground">Total a ingresar</span><span className="font-mono font-bold text-lg">5.356,50 €</span></div>
        ...
        <TabsContent value="modelo190" className="mt-4">
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div><CardTitle className="text-base">Modelo 190 — Resumen anual</CardTitle><CardDescription>Declaración informativa anual de retenciones e ingresos a cuenta del IRPF</CardDescription></div>
                <Button size="sm" variant="outline" disabled><FileText className="h-4 w-4 mr-1" /> Disponible en T4</Button>
              </div>
            </CardHeader>
            <CardContent>
                ...
                {DEMO_EMPLOYEES.map((emp) => (
                  <div key={emp.id} className="flex items-center justify-between p-3 rounded-lg border">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Retención acum.: {(emp.retencionMensual * 6).toFixed(2)} €</p>
                    </div>
                    <Button variant="ghost" size="sm" onClick={() => toast.success(`Certificado de ${emp.name} generado`)}><Download className="h-4 w-4 mr-1" /> PDF</Button>
                  </div>
                ))}
              </div>
            </CardContent>
