 ...
 * @shared-legal-core — Ownership: Shared Legal Core
 *
 * Parses legal citations (Art. 55 ET, Art. 56.1 LGSS, etc.) and generates
 * URLs to official legal databases (BOE, EUR-Lex, BOPA, etc.)
 *
 * Migrado desde src/utils/legalReferenceResolver.ts en F2.
 * Consumidores legacy importan via re-export en la ubicación original.
 */

export interface LegalLinkResult {
  original: string;
  label: string;
  url: string;
  source: string;
