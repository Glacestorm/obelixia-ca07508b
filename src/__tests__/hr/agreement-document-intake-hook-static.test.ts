/**
 * B13.2 — Static guards on the Document Intake hook.
 */
...
const HOOK = readFileSync(
  resolve(process.cwd(), 'src/hooks/erp/hr/useAgreementDocumentIntake.ts'),
  'utf8',
);

describe('B13.2 — Document Intake hook static guards', () => {
  it('1. hook delegates to authSafeInvoke (which uses functions.invoke)', () => {
    expect(HOOK).toMatch(/authSafeInvoke/);
    expect(HOOK).toMatch(/erp-hr-agreement-document-intake/);
    // Hook does not call other edges
    expect(HOOK).not.toMatch(/functions\.invoke\(['"]erp-hr-(?!agreement-document-intake)/);
  });
